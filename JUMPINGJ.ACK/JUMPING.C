...
init_resources()
{
    if (!rsrc_load("jumping.rsc")) {
      form_alert(1, "[0][Cannot find jumping.rsc file][Bye]");
      exit(2);
    }
}
...
do_menu(int *message)
{
    ...
    switch(itemid) {
        case ABOUT:
            do_dialog(info);
            info[INFOOK].ob_state&=~SELECTED;
        break;
        case QUIT:
            end=form_alert(2, "[1][Are you sure?][Cancel| Yes ]");
            if(end==2){
                shutdown(0);
            }
        break;
    }
    menu_tnormal(menu, menuid, 1);
}
...
do_dialog(OBJECT *dialog)
{
    ...
    form_dial(FMD_START, 0, 0, 0, 0, x, y, w+2, h+2);
    objc_draw(dialog, 0, 10, x, y, w+2, h+2);
    draw_help(x+3,y+3);
    itemhit = form_do(dialog, 0);
    form_dial(FMD_FINISH, 0, 0, 0, 0, x, y, w+2, h+2);
...
draw_help(int x,int y){
    ...
    pxyarray[6]=x+239; pxyarray[7]=y+155;
...
void draw_proc(int x, int y, int w, int h)
{
    ...
    if(helpflag) draw_help(xwork+125, ywork+71);
    pxyarray[0]=xwork; pxyarray[1]=ywork;
...
multi()
{
    int event, br, control=20, i, Start=TRUE;
            ...
            if(helpflag){
                ...
                if (hidden && event & MU_TIMER){
                    ...
                    if (fallflag){
                        ...
                        if (mx>mxold+5 || ((mx==maxx)&&walk) || walk==1){ walk=1; go_right(&jack.x);}
                        if (mx<mxold-5 || ((mx==0)&&walk) || walk==2){ walk=2; go_left(&jack.x);}
                        draw_jack(jack.x, jack.y, jack.sprite);
                    }
                }
                mxold=mx;
